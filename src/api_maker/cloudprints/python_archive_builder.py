...
class PythonArchiveBuilder(ArchiveBuilder):
            ...
            try:
                if os.path.isdir(source):
                    ...
                    try:
                        ...
                        log.error(
                            f"Error writing requirements to {requirements_path}: {e}"
                        )
                        raise
            except Exception as e:
    ...
    def install_requirements(self):
        ...
        log.info(
            f"Installing packages using: {sys.executable} -m pip3 install --target {self._libs} --platform manylinux2014_x86_64 --implementation cp --only-binary=:all: --upgrade --python-version 3.9 -r {requirements_file}"
        )
